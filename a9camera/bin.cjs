...
var serveHttp = (port) => {
  const server = import_node_http.default.createServer((req, res) => {
    ...
    if (req.url.startsWith("/ui/")) {
      ...
      const ui2 = asd_default.toString().replace(/\${id}/g, devId).replace(/\${name}/g, cameraName(devId)).replace(/\${audio}/g, config2.cameras[devId].audio ? "true" : "false");
      res.end(ui2);
      return;
    }
    ...
    if (req.url.startsWith("/rotate/")) {
      ...
      Object.keys(sessions2).forEach(
        (id) => res.write(`<h2>${cameraName(id)}</h2><a href="/ui/${id}"><img src="/camera/${id}"/></a><hr/>`)
      );
      res.write("</body>");
      res.write("</html>");
      res.end();
...
yargs_default(hideBin(import_node_process.default.argv)).command(
  ...
  "configure a camera",
  (yargs) => {
    return yargs.option("log_level", { describe: "Set log level", default: "info" }).option("discovery_ip", { describe: "Camera discovery IP address", default: "192.168.238.1" }).option("ssid", { describe: "Wifi network for the camera to connect to" }).option("password", { describe: "Wifi network password" }).demandOption(["ssid", "password"]).string(["ssid", "password"]);
  },
  (argv) => {
  ...
  (yargs) => {
    return yargs.option("log_level", { describe: "Set log level", default: "info" }).option("discovery_ip", { describe: "Camera discovery IP address", default: "192.168.238.1" }).option("out", { describe: "Path for output file" }).demandOption(["out"]).string(["out", "discovery_ip"]);
  },
  (argv) => {
    buildLogger(argv.log_level, void 0);
    if (majorVersion < 16) {
